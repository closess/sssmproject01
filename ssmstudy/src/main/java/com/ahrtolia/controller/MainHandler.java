...
public class MainHandler {
    ...
    public ModelAndView getProductByType1(Integer type1Id, int page, HttpSession session, HttpServletRequest request){
        ...
        if(page<1){
            page = 1;
        }else if(page>mainService.getProductCountByType1(type1Id,8) && mainService.getProductCountByType1(type1Id,8) != 0){
            page = mainService.getProductCountByType1(type1Id,8);
        }
        ...
        // 到值栈的数据
        request.setAttribute("page",page);
        if(mainService.getProductCountByType1(type1Id,8) == 0){
        ...
        modelAndView.addObject("type1Product",productList);
        modelAndView.addObject("allPageCount",pageCount);
        int productCount = mainMapper.getProductCountByType1(type1Id);
        modelAndView.addObject("productCount",productCount);
        //设置一个标志  用于前端分页区别不同的请求
